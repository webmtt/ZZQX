...
<template>
    <div ref="workResult" class="workResult" style="padding: 10px 10px 0 10px;background: #FFF;">
        <i-form :model="serachCondtion" ref="searchPart" inline :label-width="80">
            ...
            <Form-item prop="stationName" style="margin-bottom:5px;position: absolute;left: 3px;top: 13px;">
                <i-input type="text" style="width: 270px;background-color: #FFF !important;border: 1px solid #E5E5E5;" v-model="serachCondtion.stationName" placeholder="站点名称"></i-input>
            </Form-item></div>
        ...
        <Scroll :height="tableHeight" style="width: 100%;" :loading-text="text" :on-reach-bottom="handleReachBottom">
          <Card class="card" style="margin-bottom: 2px" v-for="(data,index) in tableData" :key="index">
            ...
            <i-button style="background-color:indianred;border:none;" size="small" type="primary" v-has="'/pointReport/delete'" @click="deleteData(data)">删除</i-button>
            </div>  
            <!--</a>-->
          </Card>
        </Scroll>
      <!-- 编辑/新增作业结果的模态框 -->
        <add-or-insert-info-point class="inPageModal" @handleSucc="search(tableData.length + 1)" :modalData="modalOption"></add-or-insert-info-point>
    </div>
</template>
