...
<Modal :mask-closable="false" :closable="false" width="610px" v-model="show" :title="title">
    <Form ref="equipmentForm" inline :model="formModel" v-if="show" :label-width="100" :rules="ruleValidate">
        ...
        <FormItem label="作业站点" prop="stationName">
            <Select v-model="formModel.stationName" :label-in-value="true" style="width: 162px" @on-change="NameOptionsChanged">
...
<script>
    export default {
            ...
            updateOrAddEquipment(name) {
                this.$refs[name].validate((valid) => {
                    if (valid) {
                        ...
                        this.loading = true;
                        if(titlePreText == '新增'){
                            this.$api.insertOrUpdateConsumablesInfo(params)
                            .then(res => {
                            ...
                            this.$api.updateOrUpdateConsumablesInfo(params)
                            .then(res => {
                                this.$Message.info(`${titlePreText}成功`);
                                this.show = false;
                                this.$emit('handleSucc');
        ...
        mounted() {
             // 获得所有装备类型;
            this.$api.getAllConsumablesType()
            .then(res => {
